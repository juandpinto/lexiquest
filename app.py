...
from agents import initialize_graph
...
def get_llm(api_key_from_ui: Optional[str] = None):
    ...
    if api_key_from_ui:
        ...
        elif api_key_from_ui.startswith("AIza"):
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    def handle_start_story(current_history: List[Dict[str, str]], api_key: str):
        _app, llm_info_status = ensure_graph_initialized(api_key)

        # Gradio's chatbot with type="messages" expects List[Dict[str, str]]
        updated_history = current_history + [{"role": "user", "content": "Let's start!"}]
        updated_history = updated_history + [{"role": "assistant", "content": ""}] # Placeholder
        ...
        for ai_response_chunk in chat_interface_function("Let's start!", api_key):
            updated_history[-1]["content"] = ai_response_chunk
            yield updated_history, llm_info_status

    def handle_submit(message_text: str, current_history: List[Dict[str, str]], api_key: str):
    ...
    msg_textbox.submit(lambda: "", inputs=[], outputs=[msg_textbox]) # Clear textbox after submit
